...
int main(){
	...
	/*
		Aqui foi usada o função "struct", cujo o objetivo era criar uma forma de registra os clientes
	...
	*/
	...
	int sessoesRealizadas = 0, clientesAssistiramFilme = 0, clientesMasculino = 0, clientesFeminino = 0, maioresIdadeMasculino = 0, maioresIdadeFeminino = 0, menoresIdadeMasculino = 0, menoresIdadeFeminino = 0; //váriaveis auxíliares
	...
	//A função "printf" foi usada para mostra uma mensagem de boas vindas ao usuário
	printf("Bem-vindo ao sistema de cadastro de sessões, que ira de auxiliar no seu trabalho.\n\n");
	...
	*/
	do {
		printf("Quantas sessões serão realizadas(Maximo de 2 sessões por registro)? ");
	...
	*/
	...
	system("cls"); //função usa pra limpar a tela do usuario, para maior fluidez no uso do programa
		...
		*/
		printf("Agora ira ser exibido um relatorio completo com todos os dados informados anteriormente.\n\n");
		...
		//função "for" usada pra separar as idades dos clientes entre maior e menor de idade
		for(int i = 0; i < clientesAssistiramFilme; i++) {
			if(clientes[i].idade < 18){ // checando se é menor de idade (adulto)
				switch(clientes[i].sexo){ //função "switch" responsavel pela lógica que separa os clientes de acordo com o sexo
					case 'M':
						menoresIdadeMasculino++; //acrecenta
						break; // quebra ao "switch"
					case 'F':
						menoresIdadeFeminino++; //acrecenta
						break; // quebra ao "switch"
					default: // valor desconhecido
						printf("*SEXO NÃO INFORMADO*"); //exibe na tela do usuário
			}
	...
	printf("Nessa parte final do relatorio será informado o número de clientes separado por idade e sexo.");
	...
	printf("%d Homens menores de idade assistiram ao filme %s.\n", menoresIdadeMasculino, nomeFilme);
	printf("%d Mulheres menores de idade assistiram ao filme %s.", menoresIdadeFeminino, nomeFilme);
	printf("\n\nObrigado por informa os dados dessa sessões :D.");	
